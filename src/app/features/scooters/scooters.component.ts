...
export class ScootersComponent implements OnInit {
  ...
  // ];
    ...
    scooters: Scooter[] = [];  // Inicialize como um array vazio

  constructor(private scootersService: ScootersService) { }

  ngOnInit(): void {
    this.scootersService.list().subscribe({
      next: (scooters) => {
        this.scooters = scooters;  // Atribua os dados retornados
      },
      error: (err) => console.error('Erro ao carregar scooters', err)
    });
  }
}
